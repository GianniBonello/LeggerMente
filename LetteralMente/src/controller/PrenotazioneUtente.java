...
public class PrenotazioneUtente extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
			...
			if(request.getParameter("idLibro")!= null && Utility.trovaLibro(request.getParameter("idLibro")).getQuantita()>0) {
					Prenotazione p = new Prenotazione();
					p.setData(new Date());
					Utility.inserisciPrenotazione(p, request.getParameter("idLibro"), ((Utente)request.getSession().getAttribute("utenteLoggato")).getIdUtente());
